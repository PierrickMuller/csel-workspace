...
static ssize_t exo7_read(struct file* f,
                             char __user* buf,
                             size_t sz,
                             loff_t* off)
{
...
static unsigned int exo7_poll(struct file* f, poll_table* wait)
{
    unsigned mask = 0;
...
static struct file_operations exo7_fops = {
    .owner = THIS_MODULE,
    .read  = exo7_read,
    .poll  = exo7_poll,
};
...
static int __init exo7_init(void)
{
    ...
    pr_info("Linux module exo7 loaded(status=%d)\n", status);
    return status;
}
...
static void __exit exo7_exit(void)
{
    ...
    pr_info("Linux module exo7 unloaded\n");
}
...
MODULE_DESCRIPTION("Module exo7");
MODULE_LICENSE ("GPL");